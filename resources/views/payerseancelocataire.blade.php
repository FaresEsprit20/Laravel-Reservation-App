...
<form action="{{ route('payer.locataire.update') }}" method="POST" class="row g-3" id="creategroup">
    @method('PUT')
    @csrf     
      <input name="locataire" type="hidden" value="{{ $locataire }}" class="form-control" id="id_eleve" >
    ...
    @if ($message = Session::get('locataire_seance_paid_success'))
